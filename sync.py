...
reload(sys)
# Python3字符串默认编码unicode, 所以sys.setdefaultencoding也不存在了 删除该行代码
...
class my_db():
    ...
    def getConnection(self):
        # conn = pymysql.connect('localhost', 'root', 'root', 'lucky')
...
class mark_log:
    ...
    def open_file(self):
        try:
            ...
            self.config.read_file(open(self.file, 'r+'))
        except Exception as error:
            logger.error("%s 文件打开错误" % (self.file))
            logger.error(error)
            exit(1)
    ...
    for data in tmp_data:
        for key, value in data['values'].items():
            if value == datetime.datetime(1970, 1, 1, 0, 0):
                ...
                # data['values'][key] = '1970-01-02 14:01:00'
            # 处理mysql里面是Null的问题
            if value is None:
...
def event_primary_key(schema, table, tmp_data, pk_dict):
    ...
    for data in tmp_data:
        for k, v in data['values'].items():
            ...
            if type(v) == datetime.datetime:
                ...
                data_dict[k] = str(v)
            # Python3统一了str和unicode类型，因此不再有内置的unicode强制转换操作符。
            # elif type(v) == unicode:
    ...
    if primary_key:
        ...
        for key in list(last_del.keys()):
            if key not in primary_key:
                del last_del[key]
        # for key, item in last_del.items():
        #     print(key)
        #     print(item)
        #     k, v = item
        #     if k not in primary_key:
...
def data_to_ck(event, alarm_info, alarm_mail, debug, skip_dmls_all, skip_delete_tb_name, skip_update_tb_name, pk_dict,
    ...
    # 字段大小写问题的处理
    for data in event:
        # data是字典，原写法python3会报错 RuntimeError: dictionary changed size during iteration
        for items in data:
            ...
            for item in list(items['values'].items()):
                key, value = item
                # print(key)
                # print(value)
                if colum_lower_upper:
                    items['values'][key.upper()] = value
                else:
                    items['values'][key.lower()] = value
...
def init_parse():
    ...
    parser.add_argument('-c', '--conf', required=False, default='./env/local/metainfo.conf',
                        help='配置文件路径，如果传了此参数则--env无效')
    parser.add_argument('-e', '--env', required=False, help='设置不同的运行环境 local/dev/prod')
    parser.add_argument('-d', '--debug', action='store_true', default=False, help='是否debug，传此参数则显示执行sql')
...
if __name__ == '__main__':
    ...
    if args.env is not None:
        config = './env/%s/metainfo.conf' % args.env
